...
from moment.utils.classification_metrics import get_classification_metrics
...
class RFClassificationFinetuning(Tasks):
    ...
    def validation(self, data_loader, return_preds: bool = False):
        ...
        with torch.no_grad():
            for batch_x in tqdm(data_loader, total=len(data_loader)):
                ...
                class_label = batch_x.class_label.float().to(self.device)
                ...
                if return_preds:
                    trues.append(class_label.detach().cpu().numpy())
    ...
    def train(self):
        ...
        while cur_epoch < self.args.max_epoch:  # Epoch based learning only
            ...
            for batch_x in tqdm(
                ...
                loss = self.criterion(outputs.forecast, class_label)
    ...
    def compute_classification_metrics(self, opt_steps: int):
        ...
        metrics = get_classification_metrics(y=trues, y_hat=preds, reduction="mean")
